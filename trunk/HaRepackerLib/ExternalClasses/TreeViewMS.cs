...
//Published on CodeProject by Stephane Rodriguez
...
namespace HaRepackerLib
{
    ...
    public class TreeViewMS : TreeView
    {
        ...
        private TreeNode currParent = null;
...
this.SetDoubleBuffered();
        ...
        public override System.Drawing.Color BackColor
        {
            ...
            set
            {
                backColorArgb = value.ToArgb();
                base.BackColor = value;
            }
        }

        protected void paintNode(TreeNode n)
        {
        ...
        private bool IsBackColor(Color a)
        {
            int argb = a.ToArgb();
            return argb == 0 || argb == backColorArgb;
        }

        private void SelectNode(TreeNode n, TreeViewAction action)
        {
            if (IsBackColor(n.BackColor))
            {
                TreeViewCancelEventArgs e = new TreeViewCancelEventArgs(n, false, action);
                base.OnBeforeSelect(e);
                if (e.Cancel) return;
                SelectedNodes.Add(n);
                paintNode(n);
                base.OnAfterSelect(new TreeViewEventArgs(n, action));
            }
        }

        private void UnselectNode(TreeNode n, TreeViewAction action)
        {
            if (!IsBackColor(n.BackColor))
            {
                TreeViewCancelEventArgs e = new TreeViewCancelEventArgs(n, false, action);
        ...
        protected override void OnMouseDown(MouseEventArgs e)
        {
            if (SelectedNodes.Count == 0)
            {
                ...
                if (SelectedNode != null)
                    SelectNode(SelectedNode, TreeViewAction.ByMouse);
            }
            else if (GetAsyncKeyState(Keys.ControlKey))
            {
                TreeNode n = GetNodeAt(e.X, e.Y);
                if (n != null && n.Parent == SelectedNodes[0].Parent)
                {
                    if (n.BackColor == BackColor)
                        SelectNode(n, TreeViewAction.ByMouse);
                    else
                        UnselectNode(n, TreeViewAction.ByMouse);
                }
            }
            else if (GetAsyncKeyState(Keys.ShiftKey))
            {
                TreeNode n = GetNodeAt(e.X, e.Y);
                if (n != null && n.Parent == SelectedNodes[0].Parent)
                {
                    SelectNode(n, TreeViewAction.ByMouse);
                    TreeNode curr = SelectedNode.NextNode;
                    while (curr != n)
                    {
                        SelectNode(curr, TreeViewAction.ByMouse);
                        curr = curr.NextNode;
                    }
                }
            }
