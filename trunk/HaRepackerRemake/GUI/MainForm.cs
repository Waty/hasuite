...
namespace HaRepacker.GUI
{
    public partial class MainForm : Form
    {
        ...
        public MainForm(string wzToLoad, bool usingPipes)
        {
            ...
            if (!usingPipes) return;

            try
            {
                IInterProcessConnection clientConnection = null;
                clientConnection = new ClientPipeConnection("HaRepacker4", ".");
                clientConnection.Connect();
                if (wzToLoad != null)
                    clientConnection.Write(wzToLoad);
            ...
            catch
            {
                try
                {
                    Program.PipeManager = new PipeManager("HaRepacker4", new PipeRequestHandler(OnPipeRequest));
                    Program.PipeManager.Initialize();
                }
                catch
                {
                    Warning.Error("Could not initialize piping system");
                }
            }
            if (wzToLoad != null && File.Exists(wzToLoad))
            {
                ...
                //Program.WzMan.OpenAWzFile(wzToLoad, encVersion, version, false, MainPanel);
            }
        }
        ...
        private void aPNGToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                SaveFileDialog dialog = new SaveFileDialog() { Title = "Select where to save the animation", Filter = "Animated Portable Network Graphics (*.png)|*.png|CompuServe Graphics Interchange Format (*.gif)|*.gif" };
                if (dialog.ShowDialog() != DialogResult.OK) return;
                AnimationBuilder.ExtractAnimation((WzSubProperty)MainPanel.DataTree.SelectedNode.Tag, dialog.FileName, dialog.FilterIndex == 1, UserSettings.UseApngIncompatibilityFrame);
            }
        }
        ...
        {
            WzNode child = WzNode.GetChildNode(parent, node.Name);
            if (child != null)
            {
                if (ShowReplaceDialog(node.Name))
                    child.Delete();
                else return;
            }
            parent.AddNode(node);
